...
namespace BingoFlashboard.View
{
    ...
    public partial class CallerWindow : Window
    {
        ...
        public async void VerifyCard(string cardnum)
        {
            if(cardnum is null)
            {
                cardnum = VerifyTxtBox.Text;
            }
            if (App.SharedVerificationPage is not null)
            {
                App.SharedVerificationPage.SelectCard(cardnum);
                App.SharedVerificationPage2.SelectCard(cardnum);
                VerifyTxtBox.Text = "";
